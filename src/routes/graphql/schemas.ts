...
const UserType: GraphQLObjectType = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    userSubscribedTo: {
      ...
      // resolve(parent) {
      //   return prisma.user.findUnique({ where: { id: parent.id } }).userSubscribedTo();
      // }
      ...
      resolve: async (parent: { id: string }) => {
        const subscriptions = await prisma.user
          ...
          .userSubscribedTo();
          if (subscriptions) {
            // const res = subscriptions.map(async (el) => await prisma.subscribersOnAuthors
            // .findUnique({
            //   where: {
      ...
      resolve: async (parent: { id: string }) => {
        const subscribers = await prisma.user
          ...
          if (subscribers) {
            ...
            // const res = subscribers.map(async (el) => await prisma.subscribersOnAuthors
            // .findUnique({
            //   where: {
            //     subscriberId_authorId: {
            //       subscriberId: el.subscriberId,
